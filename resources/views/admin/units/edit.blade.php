<x-app-layout>
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-900">Edit Unit Kerja</h2>
        <p class="text-gray-500 mt-1">Ubah informasi unit kerja / bagian.</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 max-w-4xl">
        ...
        <form action="{{ route('admin.units.update', $unit->id) }}" method="POST" class="p-6 sm:px-8 sm:py-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="kode" id="kode" value="{{ old('kode', $unit->kode) }}" class="w-full rounded-lg border-gray-300 focus:border-[#055a40] focus:ring-[#055a40] shadow-sm transition-colors text-gray-500" required placeholder="Contoh: BK-03">
                ...
                <div class="space-y-2">
                    ...
                    <select name="parent_id" id="parent_id" class="w-full rounded-lg border-gray-300 focus:border-[#055a40] focus:ring-[#055a40] shadow-sm transition-colors text-gray-900">
                        ...
                        @foreach(\App\Models\UnitKerja::whereNull('parent_id')->orWhere('level', 'biro')->where('id', '!=', $unit->id)->get() as $p)
                            <option value="{{ $p->id }}" {{ old('parent_id', $unit->parent_id) == $p->id ? 'selected' : '' }}>{{ $p->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama', $unit->nama) }}" class="w-full rounded-lg border-gray-300 focus:border-[#055a40] focus:ring-[#055a40] shadow-sm transition-colors text-gray-500" required placeholder="Contoh: Bagian Perencanaan Anggaran">
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="3" class="w-full rounded-lg border-gray-300 focus:border-[#055a40] focus:ring-[#055a40] shadow-sm transition-colors text-gray-500" placeholder="Jelaskan tugas pokok dan fungsi unit kerja ini...">{{ old('deskripsi', $unit->deskripsi) }}</textarea>
