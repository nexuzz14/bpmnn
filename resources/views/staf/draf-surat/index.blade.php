<x-app-layout>
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <a href="{{ route('staf.buat-surat.index') }}" class="px-4 py-2 bg-white border border-gray-200 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors shadow-sm flex items-center gap-2">
    ...
    <!-- Filters & Search -->
    <form action="{{ route('staf.draf-surat.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 mb-6">
        <div class="flex-1">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center pt-4 border-t border-gray-100 mt-2 gap-4">
                ...
                <div class="flex gap-2">
                    ...
                    @if($latestReviu && $latestReviu->status == 'revisi' && $draf->status === 'revisi')
                    <a href="{{ route('staf.draf-surat.edit', $draf) }}" class="px-4 py-1.5 bg-[#701a35] text-white font-bold rounded text-xs hover:bg-[#5b152b] transition-colors shadow-sm">
                        Edit Draft
                    </a>
                    @else
                    <a href="{{ route('staf.draf-surat.show', $draf) }}" class="px-4 py-1.5 bg-white border border-[#701a35] text-[#701a35] font-bold rounded text-xs hover:bg-gray-50 transition-colors shadow-sm">
                        Lihat Detail
                    </a>
                    @endif
                </div>
