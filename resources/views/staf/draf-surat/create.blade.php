<x-app-layout>
    ...
    <div class="bg-[#fefce8] border-l-4 border-[#701a35] rounded-r-xl p-2 MB-6 shadow-sm">
        ...
        <div class="italic text-[#92400e] text-sm mb-3">
            @if($disposisi->catatan)
                "{{ $disposisi->catatan }}"
            @else
                <span class="text-[#b45309]">Tidak ada catatan tambahan.</span>
            @endif
        </div>
        <div class="text-sm text-[#92400e]">
            Deadline: {{ \Carbon\Carbon::parse($disposisi->tenggat_waktu)->translatedFormat('d M Y') }} &nbsp;&nbsp;&nbsp; Prioritas: {{ \Carbon\Carbon::parse($disposisi->tenggat_waktu)->diffInDays(now()) <= 1 ? 'Tinggi' : 'Normal' }}
        </div>
    </div>
    ...
    @if(session('error'))
        <div class="bg-red-50 text-red-800 p-4 rounded-xl border border-red-200 flex items-start gap-3 mb-6">
            ...
            <div>
                <h4 class="text-sm font-bold mb-1">Gagal mengajukan draf</h4>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-8">
        ...
        <div class="p-6">
            ...
            <div id="template-download-wrapper" class="hidden mb-4 p-4 bg-[#fdf2f8] rounded-lg border border-[#fce7f3]">
                ...
                <a id="downloadTemplateBtn" href="#" class="inline-flex items-center px-4 py-2 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-[#701a35] hover:bg-[#5b152b] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#701a35]">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-6">
            <form id="form-draf-surat" action="{{ route('staf.draf-surat.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" name="perihal" class="w-full px-4 py-2 bg-white border border-gray-200 rounded-lg focus:ring-[#701a35] focus:border-[#701a35] shadow-sm text-sm" value="{{ old('perihal', $disposisi->suratMasuk->perihal ?? '') }}" placeholder="Masukkan perihal surat">
                </div>
