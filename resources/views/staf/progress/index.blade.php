                        ...
                        @php
                            ...
                            if ($latestReview) {
                                if ($latestReview->tingkat == '1') {
                                    if ($latestReview->status == 'revisi') {
                                        $step2 = 'revisi';
                                        $status_text = 'Draft Dikembalikan';
                                        $status_bg = 'bg-[#fee2e2] text-[#dc2626]';
                                    } else {
                                        $status_text = 'Menunggu Review Kasubtim';
                                    ...
                                    if ($latestReview->status == 'revisi') {
                                        $step3 = 'revisi';
                                        $status_text = 'Draft Dikembalikan (Kabag)';
                                        $status_bg = 'bg-[#fee2e2] text-[#dc2626]';
                                    } else {
                                        $status_text = 'Menunggu Review Kabag';
                                        $status_bg = 'bg-indigo-100 text-indigo-800';
                                        if ($latestReview->status == 'disetujui') {
                                            $step3 = true;
                                        }
                                    ...
                                    if ($latestReview->status == 'revisi') {
                                        $step4 = 'revisi';
                                        $status_text = 'Draft Dikembalikan (Kabiro)';
                                        $status_bg = 'bg-[#fee2e2] text-[#dc2626]';
                                    } else {
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-1.5">
                                <span class="px-2.5 py-1 rounded text-[10px] font-bold bg-[#065f46] text-white">Konsep</span>
                                <div class="w-4 h-0.5 {{ $step2 ? 'bg-[#065f46]' : 'bg-[#e5e7eb]' }}"></div>
                                <span class="px-2.5 py-1 rounded text-[10px] font-bold {{ $step2 === true ? 'bg-[#065f46] text-white' : ($step2 === 'revisi' ? 'bg-[#dc2626] text-white' : 'bg-[#e5e7eb] text-[#9ca3af]') }}">Review Kasubtim</span>
                                <div class="w-4 h-0.5 {{ $step3 ? 'bg-[#065f46]' : 'bg-[#e5e7eb]' }}"></div>
                                <span class="px-2.5 py-1 rounded text-[10px] font-bold {{ $step3 === true ? 'bg-[#065f46] text-white' : ($step3 === 'revisi' ? 'bg-[#dc2626] text-white' : 'bg-[#e5e7eb] text-[#9ca3af]') }}">Review Kabag</span>
                                <div class="w-4 h-0.5 {{ $step4 ? 'bg-[#065f46]' : 'bg-[#e5e7eb]' }}"></div>
                                <span class="px-2.5 py-1 rounded text-[10px] font-bold {{ $step4 === true ? 'bg-[#065f46] text-white' : ($step4 === 'revisi' ? 'bg-[#dc2626] text-white' : 'bg-[#e5e7eb] text-[#9ca3af]') }}">Kabiro</span>
                                <div class="w-4 h-0.5 {{ $step5 ? 'bg-[#065f46]' : 'bg-[#e5e7eb]' }}"></div>
                                <span class="px-2.5 py-1 rounded text-[10px] font-bold {{ $step5 === true ? 'bg-[#065f46] text-white' : 'bg-[#e5e7eb] text-[#9ca3af]' }}">Selesai</span>
                            </div>
                        </td>
                        @if($role !== 'tata_usaha')
                            <td class="px-6 py-4 text-center">
                                ...
                                @elseif($role === 'staf')
                                    <a href="{{ route('staf.draf-surat.show', $draf->id) }}" class="text-[#701a35] hover:text-[#5b152b] text-[13px] font-bold block leading-tight">
                                        Lihat<br>Detail
                                    </a>
                                @else
