<x-app-layout>
    <div class="mb-6 flex items-center justify-between">
        <div class="flex items-center gap-4">
            <a href="{{ route('kabag.menunggu-kabiro.index') }}" class="w-10 h-10 bg-white border border-gray-200 rounded-lg flex items-center justify-center text-gray-500 hover:bg-gray-50 hover:text-gray-700 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            </a>
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Detail Surat Menunggu Kabiro</h2>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <p class="text-gray-900">{{ $reviuSurat->drafSurat->suratMasuk->asal_surat }}</p>
                    </div>
                    <div>
                        <span class="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">TANGGAL TERIMA</span>
                        <p class="text-gray-900">{{ \Carbon\Carbon::parse($reviuSurat->drafSurat->suratMasuk->tanggal_terima)->translatedFormat('d F Y') }}</p>
                    </div>
                    <div>
                        <span class="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">PERIHAL</span>
                        <p class="text-gray-900">{{ $reviuSurat->drafSurat->suratMasuk->perihal }}</p>
                    </div>
                    <div>
                        <span class="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">NOMOR DRAF</span>
                        <p class="text-gray-900">DRAFT - {{ str_pad($reviuSurat->drafSurat->id, 3, '0', STR_PAD_LEFT) }}/{{ now()->year }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <p class="text-gray-900">{{ $reviuSurat->drafSurat->pembuat->name ?? '-' }}</p>
                    </div>
                    <div>
                        <span class="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">DARI SUB TIM</span>
                        <p class="text-gray-900">{{ $reviuSurat->drafSurat->penugasan->dariUser->name ?? '-' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="relative">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-2">
                            ...
                            <span class="text-xs text-gray-400 mt-1 sm:mt-0 font-medium">
                                {{ \Carbon\Carbon::parse($reviuSurat->drafSurat->created_at)->translatedFormat('d M Y H:i') }}
                            </span>
                        </div>
                        <div class="bg-gray-50 border border-gray-100 rounded p-4">
                            ...
                            <div class="mt-3 text-xs text-gray-500">
                                Oleh: <span class="font-medium text-gray-900">{{ $reviuSurat->drafSurat->pembuat->name ?? '-' }}</span>
                            </div>
                        </div>
                    </div>

                    <!-- Iterasi Reviu -->
                    @foreach($timelineReviews as $reviu)
                        <div class="relative">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[600px]">
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                    ...
                    <a href="{{ asset('storage/' . $reviuSurat->drafSurat->file_draf) }}" target="_blank" class="inline-flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-200 rounded text-xs font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors shadow-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                        Buka di Tab Baru
                    </a>
                </div>
                <div class="flex-1 bg-gray-100 p-4">
                    <iframe src="{{ asset('storage/' . $reviuSurat->drafSurat->file_draf) }}" class="w-full h-full rounded border border-gray-200 shadow-sm bg-white"></iframe>
                </div>
            </div>
        </div>
    </div>
