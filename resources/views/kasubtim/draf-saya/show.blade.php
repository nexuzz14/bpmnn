<x-app-layout>
    <div class="mb-6">
        <a href="{{ url()->previous() !== url()->current() ? url()->previous() : route('kasubtim.draf-saya.index') }}" class="inline-flex items-center px-4 py-1.5 bg-white border border-gray-200 text-gray-700 font-bold rounded hover:bg-gray-50 transition-colors shadow-sm mb-4 gap-2 text-sm">
    ...
    <div class="flex justify-between items-center mb-8">
        <form action="{{ route('kasubtim.draf-saya.destroy', $drafSurat->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus draft ini?');">
            @csrf
            @method('DELETE')
        ...
        <div class="flex gap-4">
            <a href="{{ route('kasubtim.draf-saya.index') }}" class="px-6 py-2.5 bg-white border border-gray-300 text-gray-700 font-bold rounded-lg hover:bg-gray-50 transition-colors shadow-sm text-sm">
                Tutup
            </a>
            @if($latestReviu && $latestReviu->status == 'revisi' && $drafSurat->status === 'revisi')
            <a href="{{ route('kasubtim.draf-saya.edit', $drafSurat->id) }}" class="px-6 py-2.5 bg-[#701a35] text-white font-bold rounded-lg hover:bg-[#5b152b] transition-colors shadow-sm text-sm">
                Edit Draft
            </a>
            @endif
        </div>
