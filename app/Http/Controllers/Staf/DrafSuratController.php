...
class DrafSuratController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('staf.draf-surat.index', compact('drafSurats', 'stats'));
    }

    public function create(Request $request)
    {
        $disposisiId = $request->query('disposisi_id');
        if (!$disposisiId) {
            return redirect()->route('staf.buat-surat.index')->with('error', 'Silakan pilih tugas disposisi terlebih dahulu.');
        }

        $disposisi = Disposisi::with('suratMasuk')->findOrFail($disposisiId);
        
        // Cek kalau sudah dibaca, ubah status
        ...
        return view('staf.draf-surat.create', compact('disposisi', 'templates'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            DB::transaction(function() use ($request, $validated) {
                ...
                $disposisi = Disposisi::findOrFail($validated['disposisi_id']);
                ...
                ReviuSurat::create([
                    ...
                    'tingkat'       => '1',
                    'status'        => 'menunggu',
                    'reviewer_id'   => $disposisi->dari_user_id
                ]);

                // Ubah status disposisi jadi ditindaklanjuti
                $disposisi->update(['status' => 'ditindaklanjuti']);
            });

            return redirect()->route('staf.draf-surat.index')->with('success', 'Draf surat berhasil diunggah dan dikirim untuk reviu Kasubtim.');
    ...
    public function show(DrafSurat $drafSurat)
    {
        ...
        return view('staf.draf-surat.show', compact('drafSurat'));
    }
    ...
    public function update(Request $request, DrafSurat $drafSurat)
    {
        ...
        DB::transaction(function() use ($request, $drafSurat) {
            ...
            // Cari disposisi untuk ambil reviewer_id (Kasubtim)
            $disposisi = \App\Models\Disposisi::where('surat_masuk_id', $drafSurat->surat_masuk_id)
                ->where('ke_user_id', $drafSurat->dibuat_oleh)
                ->first();

            // Buat Reviu Tingkat 1 lagi setelah revisi
            \App\Models\ReviuSurat::create([
                'draf_surat_id' => $drafSurat->id,
                'tingkat' => '1',
                'status' => 'menunggu',
                'reviewer_id' => $disposisi ? $disposisi->dari_user_id : null
            ]);
        });

        return redirect()->route('staf.draf-surat.index')->with('success', 'Draf surat revisi berhasil diunggah.');
    }
    ...
    public function destroy(DrafSurat $drafSurat)
    {
        ...
        return redirect()->route('staf.draf-surat.index')->with('success', 'Draft surat berhasil dihapus.');
    }
}
